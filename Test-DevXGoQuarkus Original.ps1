...
if ($intellijInstallTime) {
    Write-Log "IntelliJ installation tid: $([math]::Round($intellijInstallTime.TotalSeconds, 2)) sekunder ($($intellijInstallTime.ToString('mm\:ss')))" -Level Success
}
if ($gitInstallTime) {
    Write-Log "Git installation tid: $([math]::Round($gitInstallTime.TotalSeconds, 2)) sekunder ($($gitInstallTime.ToString('mm\:ss')))" -Level Success
}
if ($cloneTime) {
    Write-Log "Git clone tid: $([math]::Round($cloneTime.TotalSeconds, 2)) sekunder ($($cloneTime.ToString('mm\:ss')))" -Level Success
}
