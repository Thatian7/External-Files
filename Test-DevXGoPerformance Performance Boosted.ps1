...
.PARAMETER AxisUrl
    ...
    Standard: https://drive.google.com/file/d/1YN6HS62uJINanyiBW1BKMYstT9LN1KCi/view?usp=sharing
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$AxisUrl = "https://codeload.github.com/apache/axis-axis2-java-core/zip/refs/heads/master",
    
    [Parameter(Mandatory = $false)]
    [int]$WarmBuildIterations = 1,
...
Write-Log "=== BD Maskine Performance Test ===" -Level Info
...
$axisZip = "axis-axis2-java-core-master.zip"
...
$axisFolder = Join-Path $TestFolder "axis-axis2-java-core-master"
$downloadFolder = Join-Path $TestFolder "Software Download"
$axisZipPath = Join-Path $downloadFolder $axisZip
...
$coldBuildTime = Measure-Operation -Name "Maven clean package (cold cache)" -Operation {
    & mvn clean package -T 4C 2>&1 | Out-Null
}
...
$resultsFile = Join-Path $TestFolder "test-results.txt"
$results = @"
