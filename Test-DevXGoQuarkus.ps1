...
# Opret test results folder
if (-not (Test-Path $testResultsFolder)) {
    New-Item -Path $testResultsFolder -ItemType Directory -Force | Out-Null
    Write-Log "Oprettet test results folder: $testResultsFolder" -Level Success
}
...
# Opret IntelliJ vmoptions fil hvis CleanInstall er sat
if ($CleanInstall) {
    Write-Log "" -Level Info
    ...
    # Opret vmoptions fil
    $vmoptionsContent = "-Xmx8192m"
    
    try {
        Set-Content -Path $vmoptionsPath -Value $vmoptionsContent -Encoding UTF8 -NoNewline
...
"@
    ...
    try {
        ...
        Write-Log "Thread count sat til 2" -Level Info
    }
    catch {
...
if ($intellijInstallTime) {
    Write-Log "IntelliJ installation tid: $([math]::Round($intellijInstallTime.TotalSeconds, 2)) sekunder" -Level Success
}
if ($gitInstallTime) {
    Write-Log "Git installation tid: $([math]::Round($gitInstallTime.TotalSeconds, 2)) sekunder" -Level Success
}
if ($cloneTime) {
    Write-Log "Git clone tid: $([math]::Round($cloneTime.TotalSeconds, 2)) sekunder" -Level Success
}
Write-Log "IntelliJ projekt load tid: $([math]::Round($projectLoadTime.TotalSeconds, 2)) sekunder" -Level Success
Write-Log "" -Level Info
Write-Log "Setup gennemført!" -Level Success

# Gem resultater til fil
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$resultsFile = Join-Path $testResultsFolder "quarkus-setup-results_$timestamp.txt"
