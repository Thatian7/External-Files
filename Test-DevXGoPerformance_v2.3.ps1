...
.PARAMETER Cleanup
    Ryd op i test mappen efter test er færdig (beholder kun test-results filer).
    Hvis ikke sat, vil scriptet prompte brugeren.
...
.EXAMPLE
    .\Test-DevXGoPerformance.ps1 -Cleanup
    Kører test og rydder automatisk op bagefter
...
$coldBuildTime = Measure-Operation -Name "Maven clean package (cold cache)" -Operation {
    & mvn clean package -T "${mavenThreads}C" 2>&1 | Out-Null
}
...
$resultsFileName = "test-results_${timestamp}_${MavenThreadsPerCore}tpc_${WarmBuildIterations}iter.txt"
$resultsFile = Join-Path $TestFolder $resultsFileName
$results = @"
DevXGo JAVA Performance Test Resultater
========================================
...
# Hvis Cleanup parameter ikke er sat, spørg brugeren
$shouldCleanup = $Cleanup
if (-not $Cleanup) {
    Write-Log "Ønsker du at rydde op i test mappen? (test-results filer beholdes)" -Level Info
    $response = Read-Host "Ryd op? (J/N)"
    $shouldCleanup = $response -eq 'J' -or $response -eq 'j'
}

if ($shouldCleanup) {
    Write-Log "Rydder op i test mappen..." -Level Info
    Set-Location C:\
    
    # Hent alle filer og mapper undtagen test-results filer
    Get-ChildItem -Path $TestFolder -Recurse | Where-Object {
        $_.FullName -notlike "*test-results*.txt"
    } | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
    
    # Fjern tomme mapper
    Get-ChildItem -Path $TestFolder -Directory | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
    
    Set-Location $TestFolder
    
    Write-Log "Oprydning færdig - test-results filer er bevaret" -Level Success
} else {
    Write-Log "Springer oprydning over" -Level Info
}
